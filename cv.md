---
layout: page
title: "CV"
description: "Federico Cagliero — Frontend, Systems, Data/AI"
last_modified_at: 2026-06-05
---

# CV — Federico Cagliero

frontend · systems · data/ai  
📍 torino

✉️ [e-mail](mailto:federico.cagliero@gmail.com)  
🔗 [LinkedIn](https://www.linkedin.com/in/federico-cagliero-68b860a6)  
📝 [0xblog](https://0xblog.tumblr.com/)  

---

## About ✦
sr. dev + engineer across frontend, systems, data/ai.  
focus on flow, structure, clarity; ui/data/ai paths; internal tools.

---

## Roles 🧩
frontend dev · systems dev · data engineer · ai exploration · r&d

---

## Fav. Languages / Systems ⚡
js/ts · py3 · php/C · java · clj · Go · perl

🧩 [skills](./skills)

---

## Interests ✹
ui/ux flows · system design · data pipelines · llm/rag · tooling · patterns

---

## Experience — Compendium 📘
sophia antipolis r&d: js/oo, security, mv*, prototyping.  
big‑data engineering: pipelines, streaming engines, distributed compute.  
systems/devops: 24×7 ops, internal frameworks, automation.  
ai exploration: llm/rag flows, eval workflows, vector patterns.

---

## Languages 🌍
🇮🇹 italian • 🇫🇷 french • 🇬🇧 english
